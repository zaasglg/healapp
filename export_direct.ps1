# Прямой экспорт с поиском pg_dump

Write-Host "=== Поиск pg_dump ===" -ForegroundColor Green
Write-Host ""

# Поиск pg_dump в стандартных местах
$pgDumpPaths = @(
...
if (-not $pgDump) {
    ...
    Write-Host "1. Установите PostgreSQL: https://www.postgresql.org/download/windows/" -ForegroundColor Cyan
    Write-Host "2. Добавьте в PATH: C:\Program Files\PostgreSQL\15\bin" -ForegroundColor Cyan
    Write-Host "3. Используйте SQL Editor (см. export_via_sql_editor.md)" -ForegroundColor Cyan
    Write-Host ""
    exit 1
}
