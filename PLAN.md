...
**СТАТУС:** В процессе поэтапного перевода на бэкенд  
**Документ со статусом:** `docs/BACKEND_STATUS.md`

### Шаг 16.1: Аутентификация и Edge Function `accept_invite_token` (✅ ГОТОВО)
...
- [x] **ИНТЕГРАЦИЯ:** Обновлен `RegisterPage.tsx` для использования Edge Function ✅
  - [x] Регистрация сотрудников через Edge Function (`onSubmitOrgInvite`) ✅
  - [x] Регистрация клиентов через Edge Function (`ClientInviteRegisterPage.tsx`) ✅
  - [x] Убран localStorage для регистрации сотрудников и клиентов ✅
- [x] **ИСПРАВЛЕНО:** Логика обновления `caregiver_client_invite_tokens` и `organization_client_invite_tokens` с fallback на insert ✅
- [ ] **Тест/проверка:** протестировать регистрацию сотрудника и клиента через приглашение, убедиться, что данные сохраняются в Supabase.

### Шаг 16.2: Управление приглашениями (✅ ГОТОВО - на бэкенде)
- [x] RPC `generate_invite_link` с проверкой прав и лимитов. ✅
- [x] RPC `revoke_invite_link` и плановый Cron для истечения токенов. ✅
- [x] Обновлен фронтенд (`EmployeesPage`) для работы с RPC. ✅
  - [x] Использует `generate_invite_link` и `revoke_invite_link` RPC
  - [x] Загружает сотрудников из Supabase (`organization_employees`)
  - [x] Загружает приглашения из Supabase (`invite_tokens`)
- [x] **ИСПРАВЛЕНО:** `employee_role` → `role` в запросах ✅
- [x] **ИСПРАВЛЕНО:** функция `current_organization_id()` для организаций ✅
- [ ] **ТРЕБУЕТСЯ:** Применить миграцию `20251112000002_014_fix_current_organization_id.sql` на сервере
- [ ] **Тест/проверка:** протестировать создание приглашения после применения миграции

### Шаг 16.3: Дневники и доступы (✅ ГОТОВО - на бэкенде)
- [x] RPC `create_diary`: проверка уникальности, сохранение `organization_type`, создание базовых метрик. ✅
- [x] RPC `assign_employee_to_diary` / `remove_employee_from_diary`: только для патронажных агентств. ✅
- [x] RPC `share_diary_with_client` и `revoke_diary_access`. ✅
- [x] Обновлен фронтенд (`CreateDiaryPage`) для использования RPC `create_diary`. ✅
- [x] Обновлен фронтенд (`DiaryPage`, `DashboardPage`) для работы с Supabase. ✅
  - [x] `DashboardPage` загружает дневники из Supabase
  - [x] `DiaryPage` использует RPC для метрик и истории
  - [x] Загрузка метрик и значений из Supabase
  - [x] Назначение/отзыв доступа через RPC
- [ ] **ОСТАЛОСЬ:** Настройки метрик и ссылки доступа еще через localStorage (будет в Шаге 16.6)
- [ ] **Тест/проверка:** протестировать создание дневника и назначение сотрудника после применения миграций

### Шаг 16.4: Метрики и история (✅ ГОТОВО - на бэкенде)
- [x] Триггер на `diary_metric_values` для поддержания `diary_history`. ✅
- [x] RPC `save_metric_value` (поддержка pinned/custom метрик, напоминаний). ✅
- [x] RPC `get_diary_history(diary_id, date)` для вкладки «История». ✅
- [x] RPC `get_last_metric_value` для получения последнего значения метрики. ✅
- [x] Обновлен фронтенд (`DiaryPage`) для использования RPC. ✅
  - [x] Использует RPC `save_metric_value` для сохранения
  - [x] Использует RPC `get_diary_history` для загрузки истории
  - [x] Загружает метрики и значения из Supabase
- [ ] **ОСТАЛОСЬ:** Настройки времени заполнения еще через localStorage (будет в Шаге 16.6)
- [ ] **Тест/проверка:** протестировать сохранение метрик и просмотр истории после применения миграций
...
### Шаг 16.6: Интеграция фронтенда с Supabase (поэтапно)
**Порядок перевода:**
1. [x] **Регистрация и вход** (`RegisterPage.tsx`, `LoginPage.tsx`) ✅
   - [x] Использовать Edge Function `accept_invite_token` для регистрации ✅
   - [x] Убрать localStorage для регистрации сотрудников и клиентов ✅
   - [x] Вход по телефону работает через псевдо-email ✅
2. [x] **Профили** (`ProfileSetupPage.tsx`, `ProfileEditPage.tsx`) ✅
   - [x] Создание профилей через RPC `create_organization` и `create_user_profile` ✅
   - [x] Редактирование профилей через Supabase (таблицы `organizations`, `organization_employees`, `clients`) ✅
   - [x] Убран localStorage для редактирования профилей ✅
3. [x] **Карточки подопечных** (`PatientCardsPage.tsx`, `PatientCardFormPage.tsx`) ✅
   - [x] Сохранять в таблицу `patient_cards` ✅
   - [x] Убрать localStorage для карточек (кроме черновиков) ✅
4. [x] **Приглашения клиентов** (`InviteClientPage.tsx`, `ClientInviteRegisterPage.tsx`) ✅
   - [x] Регистрация клиентов через Edge Function работает ✅
   - [x] Использовать RPC `generate_invite_link` с типами `caregiver_client`/`organization_client` для создания ссылок ✅
   - [x] Убрать localStorage для создания приглашений клиентов ✅ (InviteClientPage уже использует RPC)
5. [ ] **Дневник подопечного и все операции** (`DiaryPage.tsx`, `CreateDiaryPage.tsx`, `EditDiaryMetricsPage.tsx`)
   - [x] **Загрузка данных дневника:** ✅
     - [x] Загружать данные дневника из таблицы `diaries` ✅
     - [x] Загружать карточку подопечного из таблицы `patient_cards` ✅
     - [x] Загружать метрики из таблицы `diary_metrics` ✅ (загружается из Supabase)
     - [x] Загружать значения метрик из таблицы `diary_metric_values` ✅ (загружается из Supabase)
     - [x] Загружать историю из таблицы `diary_history` ✅ (загружается через RPC `get_diary_history`)
   - [x] **Управление доступом:** ✅
     - [x] Загружать назначения сотрудников из таблицы `diary_employee_access` (уже работает через RPC) ✅
     - [x] Использовать RPC `assign_employee_to_diary` / `remove_employee_from_diary` для назначения/отзыва доступа (уже работает) ✅
     - [x] Загружать ссылки для клиентов из таблицы `diary_client_links` ✅
     - [x] Загружать внешние ссылки из таблицы `diary_external_access_links` ✅
     - [x] Использовать RPC `generate_invite_link` для создания ссылок клиентов ✅
     - [x] Создание/отзыв внешних ссылок через Supabase ✅
     - [x] Принятие приглашения клиентом через Supabase ✅
     - [x] Убрать оставшиеся использования localStorage для `diary_client_links` ✅
   - [x] **Настройки метрик:** ✅
     - [x] Сохранять настройки метрик в `diary_metrics.metadata.settings` ✅
     - [x] Загружать настройки из `diary_metrics.metadata.settings` ✅
     - [x] Убрать localStorage для `diary_metric_settings` ✅ (оставлен только fallback для обратной совместимости)
   - [x] **Редактирование метрик:** ✅
     - [x] Сохранять изменения метрик в таблицу `diary_metrics` ✅
     - [x] Создание/обновление/удаление метрик через Supabase ✅
     - [x] Сохранение настроек при редактировании метрик ✅
     - [x] Убрать fallback на localStorage для `diary_metrics` ✅
   - [x] **Сохранение значений метрик:** ✅
     - [x] Использовать RPC `save_metric_value` для сохранения ✅
     - [x] Убрать fallback на localStorage для `diary_metric_values` ✅
   - [x] **История:** ✅
     - [x] Использовать RPC `get_diary_history` для загрузки истории ✅
     - [x] Убрать fallback на localStorage для `diary_history` ✅
   - [x] **Создание дневника:** ✅
     - [x] Использовать RPC `create_diary` для создания ✅
     - [x] Загружать карточки из Supabase вместо localStorage ✅
     - [x] Проверять существующие дневники из Supabase вместо localStorage ✅
     - [x] Убрать fallback на localStorage для создания дневника ✅
   - [ ] **Редактирование показателей:**
     - [ ] Использовать RPC для обновления метрик при редактировании
     - [ ] Убрать localStorage для редактирования метрик
6. [ ] **Админ-панель** (после Шага 16.5)
   - [ ] Использовать представления и RPC из Шага 16.5
   - [ ] Убрать localStorage для админ-панели
- [ ] **Тест/проверка:** после каждого пункта тестировать функционал, убедиться что localStorage не используется
...
**Текущий этап:** ЭТАП 16 - Бэкенд — серверная логика и интеграция (в процессе)  
**Завершенные этапы:**
...
- ✅ ЭТАП 4 - Личный кабинет и профиль (завершен - базовая функциональность):
  - ✅ Шаг 4.1 - Страница заполнения профиля (завершен - на бэкенде)
  - ✅ Шаг 4.2 - Страница настроек профиля (завершен)
  - ⏳ Шаг 4.2.1 - Страница редактирования профиля (частично - создание на бэкенде, редактирование через localStorage)
  - ✅ Шаг 4.3 - Dashboard (завершен - на бэкенде)
  - ✅ Шаг 4.4 - Управление сотрудниками (завершен - на бэкенде)
  - ✅ Шаг 4.5 - Регистрация сотрудников по пригласительной ссылке (завершен - через Edge Function)
  - ✅ Шаг 4.6 - Поддержка профиля сотрудников в ProfilePage и ProfileEditPage (завершен)
  - ⏳ Шаг 4.7 - Управление клиентами (для сиделок и организаций) (частично - регистрация через Edge Function, создание ссылок через localStorage)
  - ✅ Шаг 4.8 - Регистрация клиентов по пригласительной ссылке (завершен - через Edge Function)
  - ⏳ Шаг 4.9 - Управление карточками подопечных (частично - через localStorage)
- ✅ ЭТАП 5 - Управление дневниками подопечных (завершен - базовая функциональность):
  - ✅ Шаг 5.1 - Создание дневника (завершен - на бэкенде)
  - ✅ Шаг 5.2 - Страница просмотра дневника (завершен - метрики и история на бэкенде, настройки через localStorage)
- ⏳ ЭТАП 12 - Административная панель (частично - через localStorage)
- ✅ ЭТАП 15 - Бэкенд — подготовка и миграция Supabase (завершен):
  - ✅ Все миграции созданы (18+ файлов, включая новые RPC функции)
  - ✅ RLS политики настроены
  - ✅ Индексы и производительность оптимизированы
- 🔄 ЭТАП 16 - Бэкенд — серверная логика и интеграция (в процессе):
  - ✅ Шаг 16.1 - Edge Function `accept_invite_token` (работает, интегрирован во фронтенд)
  - ✅ Шаг 16.2 - RPC функции для приглашений (созданы и интегрированы во фронтенд)
  - ✅ Шаг 16.3 - RPC функции для дневников и доступов (созданы и интегрированы во фронтенд)
  - ✅ Шаг 16.4 - RPC функции для метрик и истории (созданы и интегрированы во фронтенд)
  - ⏳ Шаг 16.5 - Админ-панель и мониторинг (не начат)
  - 🔄 Шаг 16.6 - Интеграция фронтенда с Supabase (в процессе):
    - ✅ Пункт 1: Регистрация и вход (завершен)
    - ✅ Пункт 2: Профили - создание и редактирование (завершен)
    - ✅ Пункт 3: Карточки подопечных (завершен)
    - ⏳ Пункт 4: Приглашения клиентов (частично - регистрация работает, создание ссылок через RPC для дневников, через localStorage для InviteClientPage)
    - 🔄 Пункт 5: Дневник подопечного и все операции (в процессе):
      - ✅ Управление доступом (ссылки для клиентов, внешние ссылки) - завершено
      - ⏳ Настройки метрик (diary_metric_settings -> diary_metrics.metadata.settings) - в процессе
      - ⏳ Убрать localStorage для diary_metrics, diary_metric_values, diary_history - в процессе
    - ⏳ Пункт 6: Админ-панель (не начат)

**Следующий шаг:** ЭТАП 16.6, пункт 5 - Дневник подопечного: перенос настроек метрик из localStorage в `diary_metrics.metadata.settings`

**Примечание:** Регистрация сотрудников и клиентов реализована через Edge Function `accept-invite` (полностью на бэкенде). Создание и редактирование профилей работает через Supabase (RPC функции и прямые запросы). Карточки подопечных работают через Supabase. Управление доступом в дневниках (ссылки для клиентов, внешние ссылки) работает через Supabase. Настройки метрик и некоторые операции с метриками еще используют localStorage и требуют перевода на бэкенд.

**Текущий статус приглашений:**
- ⏳ **Для частных сиделок:** Регистрация клиентов работает через Edge Function, создание ссылок еще через localStorage
- ⏳ **Для организаций:** Регистрация клиентов работает через Edge Function, создание ссылок еще через localStorage
...
- ✅ **Исправлена логика обновления invite_tokens** - добавлен fallback на insert при обновлении `caregiver_client_invite_tokens` и `organization_client_invite_tokens` с улучшенным логированием
