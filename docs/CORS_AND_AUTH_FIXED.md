...
Обновлена конфигурация nginx для всех эндпоинтов:
...
- Обработка OPTIONS запросов (preflight) с возвратом 204

### 2. Обновлен GOTRUE_URI_ALLOW_LIST ✅

Добавлен `http://localhost:5173` в список разрешенных URI:
```
SUPABASE_URI_ALLOW_LIST=http://176.124.217.224,http://localhost:5173,http://localhost:54321
```
...
- Что ключ соответствует `SUPABASE_ANON_KEY` на сервере
